    ...
    <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center justify-between border-b border-slate-200 pb-6">
        ...
        <div className="flex gap-4">
          <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-slate-200 flex items-center gap-3">
            ...
            <span className="text-lg font-black text-indigo-600">{requests.filter(r => r.status !== 'resolved').length}</span>
          </div>
        </div>
      </div>
      ...
      {categories.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
          <p className="text-slate-400 font-bold uppercase tracking-widest">No active broadcast signals detected.</p>
        </div>
      ) : (
        categories.map(category => (
          <div key={category} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {requests.filter(r => r.category === category).map(req => (
                <div key={req.id} className={`bg-white rounded-[2.5rem] border-2 transition-all overflow-hidden flex flex-col ${req.status === 'open' ? 'border-slate-200 hover:border-indigo-400 shadow-sm hover:shadow-xl' : 'border-slate-100 opacity-80'}`}>
                  ...
                  <div className="p-8 pt-0 mt-auto">
                    <button 
                      ...
                      className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-100 disabled:text-slate-400 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-indigo-100 transition-all flex items-center justify-center gap-3"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                      {req.status === 'open' ? 'I can help' : 'Signal Claimed'}
                    </button>
                    {req.status === 'offer_received' && (
                      <div className="mt-4 flex items-center justify-center gap-2">
                        <div className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse"></div>
                        <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest">Awaiting Client Approval</p>
                      </div>
                    )}
                  </div>
                </div>
